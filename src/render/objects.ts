// Typed object-draw registry.
// Extracted from vendor/ui/render-objects.js; implementations are identical.

import { wire, lighten } from "./context.js";
import { drawChest, drawTrap1, drawTrap2, drawTrap3 } from "./items.js";
import { drawStairs, drawDoorBody, drawDoorOpenBody } from "./tiles.js";

type Ctx = CanvasRenderingContext2D;

export type ObjectColors = Record<string, string>;

/** Signature shared by all object-draw functions. */
export type ObjectDraw = (
  ctx: Ctx,
  cx: number,
  cy: number,
  t: number,
...
export function drawShrine(ctx: Ctx, cx: number, cy: number, t: number, colors: ObjectColors = {}): void {
...
export function drawFountain(ctx: Ctx, cx: number, cy: number, t: number, colors: ObjectColors = {}): void {
  ...
  ctx.globalAlpha = 1;
  for (const [dx, dy, ph] of [[-7, 2, 0], [7, 0, 1.2], [-5, 5, 2.4], [6, 4, 0.8]] as [number, number, number][]) {
...
export function drawThrone(ctx: Ctx, cx: number, cy: number, _t: number, colors: ObjectColors = {}): void {
...
export const OBJECT_RENDERERS: Record<string, ObjectDraw> = {
  chest:           drawChest as unknown as ObjectDraw,
  shrine:          drawShrine,
  ...
  throne:          drawThrone,
  door_closed:     (ctx, cx, cy, _t, col) => drawDoorBody(ctx, cx, cy, col),
  door_open:       (ctx, cx, cy, _t, col) => drawDoorOpenBody(ctx, cx, cy, col),
  stairs_down:     (ctx, cx, cy, _t, col) => drawStairs(ctx, cx, cy, "down", col),
  stairs_up:       (ctx, cx, cy, _t, col) => drawStairs(ctx, cx, cy, "up",   col),
...
/**
 * Draw a dungeon object by type. Throws for unknown types.
 */
export function drawObject(
  ...
  if (!renderer) throw new Error(`drawObject: unknown type '${type}'`);
