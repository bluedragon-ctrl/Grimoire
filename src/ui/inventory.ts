...
import { getEquipmentBonuses, getItemOps } from "../items/execute.js";
...
export function mountInventoryPanel(
  ...
  function renderStats(col: HTMLElement, hero: Actor): void {
    ...
    // Permanent effects = on_hit_inflict ops from equipped items. Shown with
    // their source so the player sees where the effect comes from.
    const onHits: string[] = [];
    for (const slotKey of SLOTS) {
      ...
      for (const op of getItemOps(inst.defId)) {
        if (op.op === "on_hit_inflict") {
          onHits.push(`${def.name}: on-hit ${op.effectId} (${op.duration}t)`);
        }
      }
    }
    if (onHits.length) {
      const efxH = document.createElement("h3");
      efxH.textContent = "On-hit effects";
      col.appendChild(efxH);
      const ul = document.createElement("ul");
      ul.className = "inv-perm";
      for (const s of onHits) {
