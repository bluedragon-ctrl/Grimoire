...
function snapshotSetup(setup: RoomSetup): RoomSetup {
  return {
    room: {
      ...
      objects: (setup.room.objects ?? []).map(o => ({ ...o, pos: { ...o.pos } })),
      interiorWalls: (setup.room.interiorWalls ?? []).map(w => ({ pos: { ...w.pos } })),
      ...(setup.room.depth !== undefined ? { depth: setup.room.depth } : {}),
      ...(setup.room.archetype !== undefined ? { archetype: setup.room.archetype } : {}),
    },
